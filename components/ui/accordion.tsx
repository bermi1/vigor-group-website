...
export function Accordion({ items }: { items: AccordionItem[] }) {
  ...
  return (
    <ul className="divide-y divide-ink-900/10 overflow-hidden rounded-2xl border border-ink-900/10 bg-white">
      {items.map((item, i) => {
        const isOpen = open === i;
        return (
          <li key={item.q}>
            <h3>
              <button
                ...
                className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold-400"
              >
